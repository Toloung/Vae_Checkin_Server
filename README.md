...
许嵩 Vae+ 云服务器自动签到工具，支持多账号、企业微信等多渠道推送、菜单化配置，以及用于抢每日首签的定时预启动模式。
...
签到需要填写自己 Vae+ 账号的 Cookie，通常是登录 App 后，通过抓包工具查看自己账号请求里的 `Cookie` 请求头获得。
...
注意：Cookie 等同于账号登录凭据，不要发给别人，也不要上传到公开仓库。如果签到提示设备登录、密码风险或 Cookie 失效，请重新确认账号状态并更新 Cookie。
